...
bool LockManager::LockShared(Transaction *txn, const RID &rid) {
  ...
  KillYoungerRequest(lrq, txn, LockMode::EXCLUSIVE);
  lrq->latch_.unlock();
  std::unique_lock<std::mutex> lck(latch_);
  while (OldExists(lrq, txn->GetTransactionId(), LockMode::EXCLUSIVE) && txn->GetState() != TransactionState::ABORTED) {
    lrq->cv_.wait(lck);
  }
...
bool LockManager::LockExclusive(Transaction *txn, const RID &rid) {
  ...
  KillYoungerRequest(lrq, txn, LockMode::SHARED);
  lrq->latch_.unlock();
  std::unique_lock<std::mutex> lck(latch_);
  while (OldExists(lrq, txn->GetTransactionId(), LockMode::SHARED) && txn->GetState() != TransactionState::ABORTED) {
    lrq->cv_.wait(lck);
  }
  auto lr_iter = GetRequestInQueue(lrq, txn->GetTransactionId());
  if (txn->GetState() == TransactionState::ABORTED) {
...
bool LockManager::OldExists(LockRequestQueue *lrq, const txn_id_t &id, LockMode lower_bound) {
...
void LockManager::KillYoungerRequest(LockRequestQueue *lrq, Transaction *txn, LockMode lower_bound) {
