...
bool UpdateExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) {
  ...
  while (child_executor_->Next(&old_tuple, &old_rid)) {
    Tuple update_tuple = GenerateUpdatedTuple(old_tuple);
    ...
    table_info_->table_->UpdateTuple(update_tuple, old_rid, GetExecutorContext()->GetTransaction());
    ...
    for (IndexInfo *index : indexes_) {
      ...
      for (auto const &i : index->index_->GetKeyAttrs()) {
        updated_key_index_vals.emplace_back(update_tuple.GetValue(child_executor_->GetOutputSchema(), i));
      }
      ...
      GetExecutorContext()->GetTransaction()->GetIndexWriteSet()->emplace_back(
          old_tuple, table_info_->oid_, WType::UPDATE, Tuple(updated_key_index_vals, index->index_->GetKeySchema()),
          index->index_, GetExecutorContext()->GetCatalog());
